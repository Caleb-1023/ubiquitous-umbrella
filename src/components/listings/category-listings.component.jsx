                    ...
                    <div className="relative">
                        ...
                        {openCategory ? (
                            <ul className="absolute rounded bottom-[-1] border bg-white text-black">
                                {categories.map((c, i) => {
                                return <Link key={i} to={`/products/categories/${c}`} onClick={() => {setOpenCategory(false); getProductsByCategory(c)}}><li className="capitalize py-1 px-4 hover:bg-gray-200">{c}</li></Link>
                                })}
                            </ul>
                        ) : null}
                    </div>
