...
const AddProduct = () => {
    ...
    const getProduct = async () => {
            ...
            setLocation(response?.data?.product.location)
    }
    ...
    const handleEdit = async (e) => {
        ...
        try {
            ...
            // console.log(image)
            await axios.put(`${EDIT_PRODUCT}${productSlug}`, formData, {headers: {Authorization: `Bearer ${token}`, "Content-Type":"multipart/form-data", 'Access-Control-Allow-Origin': 'http://localhost:5173',}})
            // console.log(response)
            setLoading(false)
            setAdded(true)
        } catch(err) {
  ...
  return (
    <div className='w-screen h-screen flex items-center justify-center'>
        <div className='w-full h-[] max-w-2xl border border-gray-300 rounded-xl p-5 '>
            <h1 className='heading-text text-2xl border-b border-black'>{productSlug ? 'Edit':'Add New'} Product</h1>
            {loading ? 
            ...
            <div className='flex flex-col items-center justify-center my-4'>
                ...
                <div className='flex items-center justify-center space-x-5 my-3'>
                    <Link to={'/products'} className='p-3 rounded-lg bg-[#16294d] hover:bg-[#082a6a] text-white'>All Products</Link>
            ...
            :
            <form onSubmit={productSlug ? handleEdit : handleSubmit} className='my-3'>
                {productSlug ? 
                <div className='p-3 bg-green-200 rounded-lg w-full'>
                    <p className='text-green-900'>Please re-upload the image for the product.</p>
                </div>
                :<></>}
                {errMsg ? 
                ...
                <div>
                    ...
                    <input type="text" placeholder="Product Name" value={name} onChange={(e) => {setName(e.target.value)}} className="border-2 border-gray-400 outline-slate-500 px-3 py-2 w-full rounded-lg mb-3" />
                </div>
                <div>
                    <label htmlFor="description" className="block mb-2">Description</label>
                    <input type="text" placeholder="Description" value={description} onChange={(e) => {setDescription(e.target.value)}} className="border-2 border-gray-400 outline-slate-500 px-3 py-2 w-full rounded-lg mb-3" />
                </div>
                <div>
                    <label htmlFor="price" className="block mb-2">Price</label>
                    <input type="text" placeholder="Price" value={price} onChange={(e) => {setPrice(e.target.value)}} className="border-2 border-gray-400 outline-slate-500 px-3 py-2 w-full rounded-lg mb-3" />
                </div>
                <div>
                    ...
                    <select value={category} onChange={(e) => {setCategory(e.target.value)}} className="border-2 border-gray-400 outline-slate-500 px-3 py-2 w-full rounded-lg mb-3" >
                ...
                <div>
                    ...
                    <select value={location} onChange={(e) => {setLocation(e.target.value)}} className="border-2 border-gray-400 outline-slate-500 px-3 py-2 w-full rounded-lg mb-3" >
                        <option value=""></option>
                ...
                <input type="submit" value={productSlug ? 'Edit Product':'Add Product'} className="bg-[#0c1423] hover:bg-[#14294c] w-full text-white py-2 rounded-lg mb-2" />
            </form>}
            </>
            }
        </div>
